...
function drawPointerDown() {
  // sageata fixa sus, cu varful in jos
  const { width, height } = canvas;
  const cx = width / 2;

  // pozitionare si dimensiuni (poti regla daca vrei mai mare/mica)
  const tipY = 18;       // varful (jos)
  const baseY = 52;      // baza (sus)
  const halfW = 18;      // jumatate latime

  // umbra discreta
  ctx.save();
  ctx.beginPath();
  ctx.moveTo(cx + 1, tipY + 2);
  ctx.lineTo(cx - halfW + 1, baseY + 2);
  ctx.lineTo(cx + halfW + 1, baseY + 2);
  ctx.closePath();
  ...
  ctx.save();
  ctx.beginPath();
  ctx.moveTo(cx, tipY);                 // varf in jos
  ctx.lineTo(cx - halfW, baseY);        // colt stanga sus
  ctx.lineTo(cx + halfW, baseY);        // colt dreapta sus
  ctx.closePath();
  ctx.fillStyle = "#111";               // negru, ca sa se vada
  ctx.fill();
  ctx.restore();
}
...
spinBtn.addEventListener("click", async () => {
  ...
  // pointer e sus, varf in jos, deci vrem ca segmentul castigator sa ajunga la unghiul 3π/2
